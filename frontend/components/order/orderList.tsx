...
export default function OrderList({ updatedAt, orderItems, page, setPage, setIndex }: { updatedAt: string, orderItems: any[], page: number, setPage: Dispatch<SetStateAction<number>>, setIndex: Dispatch<SetStateAction<number | undefined>> }) {
    ...
    const maxPage: number = orderItems ? Math.ceil(orderItems.length / itemsPerPage) - 1 : 0
    ...
    return (
        <div className="w-full h-full flex flex-col 1400px:px-[80px] 1100px:px-[60px] 970px:px-[40px] px-[7.5%] 1700px:py-[80px] 1400px:py-[60px] 1100px:py-[40px] 970px:py-[20px] pb-[20px]">
            <div className="w-full 1700px:h-1/2 1100px:h-2/5 970px:h-1/3 h-1/3  flex 970px:flex-col flex-row">
                <div className="w-full 970px:h-3/4 h-full flex flex-col 970px:items-center items-start jusify-center">
                    <div className="font-roboto font-light 1700px:text-[20px] 1100px:text-[14px] text-[12px] 970px:text-[#ADADAD] text-black">Items purchased on</div>
                    <div className="970px:font-playfairDisplay font-bebasNeue 1700px:text-[40px] 1100px:text-[28px] text-[24px]">{formatted}</div>
                </div>
            ...
            <div className="w-full flex-1 1400px:py-[30px] py-[10px] border-t-[2px] 970px:border-[#E2E2E2] border-black">
            ...
            <div className="font-playfairDisplay w-full 970px:pt-[20px] border-t-[2px] 970px:border-[#E2E2E2] border-white flex-col justify-center flex flex z-1">
                <div className="flex flex-row w-full justify-between items-center 970px:mt-[10px] text-black">
                    <div className="font-roboto 1700px:text-[20px] text-[16px]">Total</div>
                    <div className="font-roboto 1700px:text-[20px] text-[16px]"> ${orderItems.reduce((sum: number, item) => sum + item.product.price * item.quantity, 0)}</div>
                </div>
            </div>
        </div>
    )
