...
export default function Header() {
    ...
    return (
        <header className="fixed top-0 right-0 w-[40px] h-[110px] flex flex-col items-center justify-around z-3">
            {accessToken ?
                <button type="button" className="h-[26px] w-[26px] duration-300 ease-in-out text-gray-300 hover:text-gray-400 cursor-pointer" onClick={async () => {
                    setAccessToken(null)
                    await signOut()
                }}>
                    <Logout />
                ...
                :
                <button type="button" className="h-[26px] w-[26px] duration-300 ease-in-out text-gray-300 hover:text-gray-400 cursor-pointer" onClick={() => router.push('/signin')}>
                    <Login />
                </button>
            }
