...
export default function SignIn({ setIsSignUp }: { setIsSignUp: Dispatch<SetStateAction<boolean>> }) {
    ...
    const emptyFields = Object.entries(signInField)
        .filter(([_, value]) => value === "")
        .map(([key]) => key as keyof typeof signInField);
    ...
    const handleSubmit = async (event: React.SubmitEvent<HTMLFormElement>) => {
        ...
        if (emptyFields.length !== 0) {
            emptyFields.map((item) => {
                ...
                const errorMessage = validateField(item, signInField[item]);
    ...
    const googleHandleClick = () => {
        window.location.href = `${process.env.NODE_ENV === 'production' ? process.env.NEXT_PUBLIC_PRODUCTION_BACKEND_URL : process.env.NEXT_PUBLIC_DEV_BACKEND_URL}/auth/google/signin/?returnUrl=${encodeURIComponent(returnUrl)}`;
    }
